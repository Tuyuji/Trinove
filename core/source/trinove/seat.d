...
class Seat : WlSeat
{
	...
	// Fired when a seat is created.
	mixin DefineEvent!("OnSeatAdded", Seat);

	// Fired just before a seat is destroyed.
	mixin DefineEvent!("OnSeatRemoved", Seat);
	...
	void setPointerFocusView(View view, Vector2 localPos)
	{
		if(view is null)
		{
			pointerFocus = PointerFocus.init;
			setPointerFocusSurface(null, Vector2(0, 0));
			updateConstraintForFocus(null, localPos);
			return;
		}
		...
		// localPos is already surface-local, find the subsurface under it.
		import trinove.surface.surface : findSurfaceAt;
		auto ss = mainSurface.computeSurfaceState();
		auto hit = findSurfaceAt(mainSurface, cast(Vector2I) ss.size, cast(Vector2I) localPos);

		pointerFocus = PointerFocus(view, hit.subsurface);
		setPointerFocusSurface(hit.surface, localPos);
		updateConstraintForFocus(pointerFocus.surface, cast(Vector2) hit.local);
	}

	// Set pointer focus to a surface with enter event at given surface-local position.
	// WARNING: This doesn't deal with subsurfaces, instead this directly sets the focus
	// for the given surface. if you want to handle subsurfaces, use setPointerFocusView and it will find the correct subsurface for you.
	void setPointerFocusSurface(WaiSurface surface, Vector2 surfaceLocalPosition)
	{
		...
		if (surface !is null && isClientValid(surface))
		{
			if (auto state = surface.client.native in _clientStates)
			{
				...
				foreach (p; state.pointers)
				{
					p.sendEnter(serial, surface, WlFixed.create(surfaceLocalPosition.x), WlFixed.create(surfaceLocalPosition.y));
					if (getResVersion(p) >= WlPointer.frameSinceVersion)
						p.sendFrame();
				}
			}
	...
	//Doens't send events, this is used when a view is destroyed pretty much.
	void clearFocusForView(View view)
	{
		if (pointerFocus.view is view)
			setPointerFocusSurface(null, Vector2(0, 0));
		if (keyboardFocusView is view)
			setKeyboardFocusSurface(null);

		if (pointerFocus.view is view)
			pointerFocus = PointerFocus();
		if(keyboardFocusView is view)
			keyboardFocusView = null;
	}
	...
	void setKeyboardFocusView(View view)
	{
		if(keyboardFocusView is view)
			return;

		auto oldView = keyboardFocusView;
		keyboardFocusView = view;
		setKeyboardFocusSurface(view !is null ? view.getSurface() : null);
		if (oldView !is null) oldView.onKeyboardFocusChanged(false);
		if (view !is null) view.onKeyboardFocusChanged(true);
	}
