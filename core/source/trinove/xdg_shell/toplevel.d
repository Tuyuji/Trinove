...
final class XdgToplevelWindow : Window
{
	...
	{
		auto wm = _protocol.xdgSurface.wmBase.compositor;
		...
		if (!_addedToWc)
		{
			...
			wm.addWindow(this);
			wm.onWindowConfigureApplied(this, Nullable!Vector2I.init);
			return true;
		}
		...
		if (_configQueue.hasAcked())
		{
			...
			if (!folded.fullscreen.isNull)
			{
				...
				this.layer = folded.fullscreen.get ? Layer.Fullscreen : Layer.Normal;
			}

			wm.onWindowConfigureApplied(this, folded.position);
		}
		else if (newSize != surfaceSize)
		{
			// Client committed a new size during interactive resize without an acked configure.
			wm.onWindowResizeCommited(this, newSize);
		}
...
final class WaiXdgToplevel : XdgToplevel, IXdgRole
{
	...
	this(WaiXdgSurface xdgSurface, WlClient cl, uint id)
	{
		...
		window.onWmCapabilitiesChanged(WmCapabilityFlags.none);
	}
	...
	override void onCommit()
	{
		...
		if (!surface.rootLocalDamage.empty)
			xdgSurface.wmBase.compositor.scheduleRepaint();
	}
	...
	{
		auto wm = xdgSurface.wmBase.compositor;
	...
	override void setTitle(WlClient cl, string t)
	{
		window.title = t;
		if (window.mapped)
			xdgSurface.wmBase.compositor.onWindowTitleChange(window);
	}
	...
	override void showWindowMenu(WlClient cl, WlResource seat, uint serial, int x, int y)
	{
		...
		auto wm = xdgSurface.wmBase.compositor;
		wm.onShowWindowMenuRequest(window, wseat, Vector2I(x, y));
	}
	...
	override void move(WlClient cl, WlResource seatRes, uint serial)
	{
		...
		auto wm = xdgSurface.wmBase.compositor;
		wm.onMoveWindowRequest(window, wseat);
	}
	...
	override void resize(WlClient cl, WlResource seatRes, uint serial, ResizeEdge edges)
	{
		...
		auto wm = xdgSurface.wmBase.compositor;
		wm.onResizeWindowRequest(window, wseat, xdgEdgeToDecorationHit(edges));
	}
	...
	{
		xdgSurface.wmBase.compositor.onMaximizeRequest(window, true);
	}

	override void unsetMaximized(WlClient cl)
	{
		xdgSurface.wmBase.compositor.onMaximizeRequest(window, false);
	}

	override void setFullscreen(WlClient cl, WlResource output)
	{
		xdgSurface.wmBase.compositor.onFullscreenRequest(window, true, null);
	}

	override void unsetFullscreen(WlClient cl)
	{
		xdgSurface.wmBase.compositor.onFullscreenRequest(window, false, null);
	}

	override void setMinimized(WlClient cl)
	{
		xdgSurface.wmBase.compositor.onMinimizeRequest(window);
	}

	private Seat findSeatForResource(WlResource seatRes)
	{
		return xdgSurface.wmBase.compositor.seatManager.findSeatForResource(seatRes);
	}
