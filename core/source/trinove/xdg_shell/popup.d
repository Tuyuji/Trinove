...
final class WaiXdgPopup : XdgPopup, IXdgRole
{
		...
		if (surface.currentBuffer)
		{
			...
			if (pendingConfigs.hasAcked())
			{
				...
				if (!popup.mapped)
				{
					...
					xdgSurface.wmBase.compositor.addPopup(popup);
				}
			...
			if (xdgSurface.wmBase && xdgSurface.wmBase.compositor)
	...
	void dismiss()
	{
		...
		if (popup.mapped)
		{
			xdgSurface.wmBase.compositor.removePopup(popup);
		}
	...
	{
		auto wseat = xdgSurface.wmBase.compositor.seatManager.findSeatForResource(seatRes);
